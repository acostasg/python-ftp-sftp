...
class Container:
    ...
    def __import_module(name):
        if Container.__container.get(name) is not None:
            return Container.__container.get(name)
        else:
            if name in Container.__mapper:
                return importlib.import_module(Container.__mapper.get(name))
            else:
